...
output: jpaRmd::render_jpr(Rmd_file = "Untitled.Rmd", Bib_file = "reference.bib")
---
...
<!-- 
このテンプレートは，１ページが６００字になるように調整されています。心理学評論は38000字以内なので，このフォーマットで63枚以内になるように執筆する。超過やその他の詳細については，執筆規定を参照ください。
http://sjpr.jp/pdf/kitei2013.pdf
-->
\thispagestyle{empty}


## Can we write a paper of the Japanese Psychological review with RMarkdown?

<!-- author-info-start -->

Yoshihiko Kunisato${}^\text{1}$・Yoshihiko　Onihige${}^\text{2}$

Senshu University${}^\text{1}$, Sakigake!! Senshu University${}^\text{2}$

<!-- author-info-end -->
<!-- 
${}^\text{1}$は，上付き文字で数字をつけるための記法です。特に考える必要はなくて，｛｝内を変更ください。author-info-startとauthor-info-endの間に著者名や所属などの情報をいれてください。これらを含めたファイルと含めてないファイルを出力します
-->

### Abstract
Can we write a paper of the Japanese Psychological Review with RMarkdown? To solve 
this mystery we headed deep into the Amazon. At the end of our long journey we found some great documents about RMarkdown and finally said, "Yes, we can". ... Well, we'll write a abstract like this.


**Key words**: RMarkdown, Reproducibility, Japanese Psychological Review
...
## RMarkdownで『心理学評論』の論文は書けるか？

<!-- author-info-start -->
国里愛彦${}^\text{1}$・鬼髭愛彦${}^\text{2}$

専修大学${}^\text{1}$, 魁!!専修大学${}^\text{2}$

謝辞:専修大学の暗黒卿教授に有益なコメントをいただきました。記して感謝の意を表します。本研究はJSPS科研費 JP12345678の助成を受けたものです。
...
RMarkdownで『心理学評論』の論文が書けるのか？この謎を解くために，私たちはアマゾンの奥深くに向かいました。数多の苦難を乗り越えた長い旅路の最後に，RMarkdownに関する素晴らしいドキュメントを見つけることができ，「はい。できます」と述べることができました。... さて，まあ，こんな感じで要約を書きます。
...
<!-- 以下は1頁が30字*20行で600字になっているかのチェック用です。
ご自身の環境でちゃんと1頁が600字になっているか確認をしてから，削除してください。-->
\clearpage
...
前のページで30字*20行で600字になっているか確認します<!-- ここまで削除-->
...
## 本研究の目的
　
　RMarkdownでちゃんと『心理学評論』に投稿できる原稿PDFファイルが作れるか検証した。
...
　論文では，図を挿入したりするのですが，その図の挿入箇所の指示を余白ですることが多いです。例えば，Figure 1をこの辺に挿入しようかなというときは，\marginpar{\fbox{\textcolor{red}{Figure 1}}}こういう感じでLatexのmarginparパッケージとfboxを活用して，Figure 1を線で囲ったものを作ります。
...
　論文では，表を挿入したりするのですが，その表の挿入箇所の指示を余白ですることが多いです。例えば，Table 1をこの辺に挿入しようかなというときは，\marginpar{\fbox{\textcolor{red}{Table 1}}}こういう感じでLatexのmarginparパッケージとfboxを活用して，Table1を線で囲ったものを作ります。
...
　散布図は，Figure 2に示す\marginpar{\fbox{\textcolor{red}{Figure 2}}}。
