...
引用文献については，このパッケージ内にあるjpa_cite()関数を使うことで日本心理学会編「論文執筆投稿の手引き」にある書式と同じ書式で出力することができます。
...
1. ヘッダー部分を2箇所修正します。
  1.　output: jpaRmd::jpa_jjpとなっているところを，output: jpaRmd::jpa_jjp2とします。
  2.　次の一行を挿入します。suppress-bibliography: yes
2. 論文を書いている**Rmdファイルの中で**，次のようにjpa_cite()関数を呼び出します。関数を呼び出す場所はどこでも結構です。
